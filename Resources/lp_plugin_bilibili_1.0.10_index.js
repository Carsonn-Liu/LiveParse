...
async function _bili_getLoginUID(authMode) {
  try {
    const resp = await _bili_authRequest({
      url: "https://api.bilibili.com/x/web-interface/nav",
      method: "GET",
...
function _bili_danmakuDriver() {
  const driver = globalThis.__biliDanmakuDriver;
  if (!driver) {
    _bili_throw("UNSUPPORTED", "bilibili danmaku driver is unavailable", {});
  }
  return driver;
}

globalThis[__bili_sharedGlobalKey] = {
  throwError: _bili_throw,
  getBuvid3And4: _bili_getBuvid3And4,
  getLoginUID: _bili_getLoginUID,
  getLiveLatestInfo: _bili_getLiveLatestInfo,
  getRoomDanmuDetail: _bili_getRoomDanmuDetail
};
...
globalThis.LiveParsePlugin = {
  ...
  async getDanmaku(payload) {
    ...
    if (!roomId) _bili_throw("INVALID_ARGS", "roomId is required", { field: "roomId" });
    const headers = await _bili_getHeaders();
    return await _bili_danmakuDriver().getDanmakuPlan(roomId, headers, "platform_cookie");
  },

  async createDanmakuSession(payload) {
    return await _bili_danmakuDriver().createDanmakuSession(payload);
  },

  async onDanmakuOpen(payload) {
    return await _bili_danmakuDriver().onDanmakuOpen(payload);
  },

  async onDanmakuFrame(payload) {
    return await _bili_danmakuDriver().onDanmakuFrame(payload);
  },

  async onDanmakuTick(payload) {
    return await _bili_danmakuDriver().onDanmakuTick(payload);
  },

  async destroyDanmakuSession(payload) {
    return await _bili_danmakuDriver().destroyDanmakuSession(payload);
  }
};
