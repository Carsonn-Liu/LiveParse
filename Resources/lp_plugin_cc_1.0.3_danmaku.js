...
const __cc_sharedGlobalKey = "__lp_plugin_cc_1_0_3_shared";
...
function _cc_buildSockJSURL(rawURL) {
  ...
  try {
    ...
    parsed = new URL(__cc_defaultWebSocketURL);
  }

  if (/\/websocket$/i.test(parsed.pathname || "")) {
  ...
  parsed.protocol = "wss:";
  parsed.pathname = `${basePath}/${_cc_pad3(Math.random() * 1000)}/${_cc_randomSessionId()}/websocket`;
  return parsed.toString();
}
