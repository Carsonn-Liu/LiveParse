...
function _cc_buildSockJSURL(rawURL) {
  const source = String(rawURL || "wss://wslink.cc.163.com/conn");
  let parsed;
  try {
    parsed = new URL(source);
  } catch (e) {
    parsed = new URL("wss://wslink.cc.163.com/conn");
  }
  if (/\/websocket$/i.test(parsed.pathname || "")) return parsed.toString();
  ...
  parsed.protocol = "wss:";
  parsed.pathname = `${basePath}/${String(Math.floor(Math.random() * 1000)).padStart(3, "0")}/${Math.random().toString(16).slice(2, 10).padEnd(8, "0")}/websocket`;
  return parsed.toString();
}

async function _cc_getDanmakuPlan(roomId) {
  const resp = await Host.http.request({
  ...
  const heartbeatInterval = String(channelData.heartbeat_interval || "60000");
  const sockJSURL = _cc_buildSockJSURL("wss://wslink.cc.163.com/conn");
  return {
    args: {
...
const __lp_cc_sharedGlobalKey = "__lp_plugin_cc_1_0_3_shared";

function _cc_danmakuDriver() {
  const driver = globalThis.__ccDanmakuDriver;
  if (!driver) {
    _cc_throw("UNSUPPORTED", "cc danmaku driver is unavailable", {});
  }
  return driver;
}

globalThis[__lp_cc_sharedGlobalKey] = {
  throwError: _cc_throw,
  userAgent: _cc_ua
};
...
globalThis.LiveParsePlugin = {
  ...
  async getDanmaku(payload) {
    const roomId = String(payload && payload.roomId ? payload.roomId : "");
    if (!roomId) _cc_throw("INVALID_ARGS", "roomId is required", { field: "roomId" });
    return await _cc_danmakuDriver().getDanmakuPlan(roomId);
  },

  async createDanmakuSession(payload) {
    return await _cc_danmakuDriver().createDanmakuSession(payload);
  },

  async onDanmakuOpen(payload) {
    return await _cc_danmakuDriver().onDanmakuOpen(payload);
  },

  async onDanmakuFrame(payload) {
    return await _cc_danmakuDriver().onDanmakuFrame(payload);
  },

  async onDanmakuTick(payload) {
    return await _cc_danmakuDriver().onDanmakuTick(payload);
  },

  async destroyDanmakuSession(payload) {
    return await _cc_danmakuDriver().destroyDanmakuSession(payload);
  }
};
